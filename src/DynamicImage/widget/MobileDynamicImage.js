...
define([
	...
	"dojo/_base/lang",
	"dojo/dom-style"
], function (declare, _WidgetBase, mxuiDom, lang, domStyle) {
	'use strict';
	...
	return declare('DynamicImage.widget.MobileDynamicImage', [_WidgetBase], {
		...
		width: 300,
		height: 300,
		defaultImage: 'ui/images/empty.gif',
		pathprefix: '',
		pathpostfix: '',
		clickmicroflow: '',
		linkattr: '',
		...
		setDataobject: function (dataobject) {
			...
			if (/\d+/.test(dataobject) && !lang.isObject(dataobject)) { //MWE: stupid case, setContext sometimes just provides us a number...
		...
		IEresize: function () {
			...
			if (origw > 0 && origh > 0) {//only apply if an valid image has been loaded
				...
				if (factor < 1) {//check prevents upscaling
					this.imageNode.width = (factor * origw) + 'px';
					this.imageNode.height = (factor * origh) + 'px';
				}
			}
		},
				...
				if (this.clickmicroflow != '') {
					mx.data.action({
						params: {
							...
							guids: [ lang.isObject(this.dataobject) ? this.dataobject.getGUID() : this.dataobject ]
						},
						error: function () {
		...
		applyContext: function (context, callback) {
			...
			if (context && !!context.getTrackId()) {
				this.subhandler = this.subscribe({ guid: context.getTrackId(), callback: lang.hitch(this, this.setDataobject) });
